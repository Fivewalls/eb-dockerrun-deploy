...
# Deploy
# CLI Front end for deployment scripts
class Deploy < Thor
  ...
  class_option(
    ...
    type: :string,
    aliases: 'f',
    desc: 'File holding variables for deployment. Anything passed as an option to the command will' \
  ...
  desc 'deploy', 'Deploy the EB dockerrun file. Will determine single or double from the file.'
  def deploy
    opts = parse_options
...
private
  ...
  def dockerrun_hash_single(opts)
    run_hash = {
  ...
  def build_zip(opts)
    ...
    Dir.mktmpdir do |tmpdir|
      ...
      s3_client.put_object(
        ...
        key: opts['bucket-key'],
        body: IO.read("#{tmpdir}/app_source.zip")
      )
    end
  end
